...
INNER JOIN [HBTSCustPerf].[rc].[SVC_ONEPAGER] as svc
    ON svc.OID = cust.OID
INNER JOIN [HBTSCustPerf].[rc].[V_HB_DENIAL_MONTHLY_METRICS] as den
	on svc.OID = den.OID
WHERE svc.METRIC_START_DATE >= CAST('2022-09-01' AS DATE)
    AND svc.METRIC_END_DATE <= CAST('2023-02-01' AS DATE)
