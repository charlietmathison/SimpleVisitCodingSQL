...
DECLARE @startDate char(10) = '2022-08-01'
...
HAVING SUM(svc.SVC_SUCCESSES_FOR_GROUPER) / NULLIF((SUM(svc.SVC_FAILURES_FOR_GROUPER) + SUM(svc.SVC_SUCCESSES_FOR_GROUPER) + SUM(svc.NOT_SVCD_FOR_GROUPER)), 0) *100 > 8
ORDER BY (SUM(svc.SVC_SUCCESSES_FOR_GROUPER) / NULLIF((SUM(svc.SVC_FAILURES_FOR_GROUPER) + SUM(svc.SVC_SUCCESSES_FOR_GROUPER) + SUM(svc.NOT_SVCD_FOR_GROUPER)), 0) * 100) desc